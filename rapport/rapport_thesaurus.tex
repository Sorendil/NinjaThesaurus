...
Dans le cadre du Master 1 Informatique à l'Université Montpellier 2, nous avons été mené à réaliser un projet de groupe pour le cours de base de données avancées dirigé par Thérèse Libourel.
	
Le projet proposé consiste à analyser et implémenter un vocabulaire partagé. Un vocabulaire partagé ou aussi appelé Thésaurus est une liste de termes organisées selon des liens qui les unissent. Les termes sont reliés entre eux par diverses relations comme la synonymie, la généralisation, la spécialisation, l'association...
...
Toute notre infrastructure, serveur web, SGBD a été installé sur une machine virtuelle Unix CentOS.

\subsection{Serveur de gestion de base de données}
Nous avons décidé d'utiliser le SGBD Oracle car tout d'abord cette base de données est orientée objet mais aussi car nous avons été formés tout au long de notre cursus sur cette base de données.

\subsection{Serveur Web}
Pour le serveur Web, nous avons utilisé Apache avec PHP5.

\subsection{Serveur de gestion de version}
Dans le contexte de gestion de projet, nous avons utilisé le serveur GitHub pour centraliser et gérer nos versions du projet afin de faciliter l'ajout, la modification et la suppression de code. 
