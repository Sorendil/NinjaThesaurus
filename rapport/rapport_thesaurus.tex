...
Nous avons donc créés une application web en PHP pour l'utilisation et la gestion des données. Nous avons utiliser le module PHP OCI pour communiquer avec la base de données Oracle.
...
Par ailleurs, dans un but de simplification d'URL, nous avons utilisé le module de réécriture d'adresse d'Apache. Ainsi, au lieu de passer des paramètres de types GET sous forme d'une syntaxe lourde pour l'utilisateur (par exemple \emph{http://domain.tld/index.php?libelle=chat}), l'utilisateur peut accéder à un libelle en tapant directement la chaine \emph{http://domain.tld/chat/} après le nom de domaine.
